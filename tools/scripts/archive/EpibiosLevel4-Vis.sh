    ...
    for sid in $(cat ${root}/params/${site}/${group}.txt); do 
      for vis in anatomy lesion; do 
        for param in small_{dti_S0,dti_FA,dti_MD}; do 
          in=cases/process/${site}/${sid}/atlas.dwi/vis/${param}_${vis}.png
          out=group/vis/${param}_${site}_${group}_${sid}_${vis}.png; 
          if [ -e ${in} ] && [ ! -e ${out} ]; then
            ln ${in} ${out}
          fi
        done 
        ...
        for param in small_{mge_mean,mge_r2star,mge_t2star}; do 
          in=cases/process/${site}/${sid}/atlas.mge/vis/${param}_${vis}.png
          out=group/vis/${param}_${site}_${group}_${sid}_${vis}.png; 
          if [ -e ${in} ] && [ ! -e ${out} ]; then
            ln ${in} ${out}
          fi

...
#setwd("/Users/rcabeen/sandbox/epibios/level4")
...
epibios.classify <- function(params, sites, time, tbi=FALSE, alpha=0.5)
{
  ...
  tryCatch(
  {
    ...
    sink(sprintf("classify-results/%s.txt", name))
    ...
    for (metric in my.metrics) 
    {
      ...
      plot.fn <- sprintf("classify-plots/%s.%s.pdf", name, metric)
    ...
    sub.out.fn <- sprintf("classify-stats/%s.csv", name)
...
dir.create(sprintf("classify-results"), showWarnings = FALSE)
dir.create(sprintf("classify-plots"), showWarnings = FALSE)
dir.create(sprintf("classify-stats"), showWarnings = FALSE)
